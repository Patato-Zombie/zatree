...
class CItemValue extends CZBXAPI {
   ...
   public function GetItemValues($itemInfo) {
			...
			if (($real_item['history'] * SEC_PER_DAY) > (time() - ($from_time + $period / 2)) // should pick data from history or trends
					&& ($period / $sizeX) <= (ZBX_MAX_TREND_DIFF / ZBX_GRAPH_MAX_SKIP_CELL)) { // is reasonable to take data from history?
		...
		foreach($itemList as $each_item){
		    ...
		    if($search){
		      ...
		      $order_result_list[$newItem['graphid']]= array("graphid"=>$newItem['graphid'],"itemname"=>$newItem['name'],"lastvalue"=>0,'min'=>0,'avg'=>0,'max'=>0,'hostname'=>$newItem['hostname']);
		    	 ...
		    	 //按关键字进行筛选
		    	if(strpos(strtolower($newItem['name']), strtolower($search_key)) === false ){
				    ...
				    if (isset($data) && isset($data['min'])) {
				    	...
				    	$order_result_list[$newItem['graphid']]= array("graphid"=>$newItem['graphid'],"itemname"=>$newItem['name'],"lastvalue"=>self::getLastValueBy($data),'min'=>min($data['min']),'avg'=>$data['avg_orig'],'max'=>max($data['max']),'hostname'=>$newItem['hostname']);
